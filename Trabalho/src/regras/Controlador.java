...
class Controlador {
	...
	public String verificaGanhador() {
		int maiorvalor = 0, ganhador = -1, somavalor=0;

		  for(int i = 0;i<getJogadores();i++) {
			  if (jogadoresFalidos[i]!=1) {
				  somavalor= pinos[i].getSaldo();
				  for(int j=0; j<36; j++) {
					  if((casas[j].getDono() == i+1) && (casas[j].getTipo() == 1)) {
						  somavalor += casas[j].getValor();
						  somavalor += ((Terreno) casas[j]).getQtdCasas()*((Terreno) casas[j]).getValorConstroiCasa();
						  somavalor += ((Terreno) casas[j]).getQtdCasas()*((Terreno) casas[j]).getValorConstroiHotel();
					  }
				  ...
				  if(somavalor >  maiorvalor) {
					  ...
					  ganhador = pinos[i].getPinoId();
				  }
			  }
		  }
		  String msg=String.format("O jogador %d ganhou  o jogo com %d reais!!",ganhador,maiorvalor);
		  return msg;
	}
